...
export const errorHandler = (
  ...
  logger.error(`[${req.method}] ${req.originalUrl} - Error: ${err.message || err}`, err);
  ...
  if (err instanceof SyntaxError && 'body' in err) {
    return sendError(res, 'Malformed JSON payload in request body', HTTP_STATUS.BAD_REQUEST, ERROR_CODES.BAD_REQUEST);
  }

  // Fallback internal error
  const message = process.env.NODE_ENV === 'production' ? 'An unexpected server error occurred' : err.message || 'Internal Server Error';
