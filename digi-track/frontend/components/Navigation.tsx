...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl bg-white shadow-[0px_-4px_16px_rgba(15,118,110,0.08)] border-t border-[#eff4ff] md:hidden">
      <div className="flex justify-around items-center px-2 py-2 w-full max-w-lg mx-auto gap-1">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center transition-all duration-200 active:scale-90 min-w-[52px] max-w-[72px] flex-1 py-1.5 px-1 rounded-xl ${
                isActive
            ...
            >
              <span
                className={`material-symbols-outlined text-[20px] mb-0.5 ${isActive ? 'fill-1 text-white' : 'text-[#3e4947]'}`}
              >
                {item.icon}
              </span>
              <span
